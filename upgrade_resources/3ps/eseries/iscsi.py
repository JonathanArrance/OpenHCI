...
class Driver(driver.ISCSIDriver):
    ...
    def initialize_connection(self, volume, connector):
        ...
        LOG.debug("iscsi_details: %s" % iscsi_details)
        iscsi_det = self._get_iscsi_portal_for_vol(vol, iscsi_details)
        LOG.debug("iscsi_det: %s" % iscsi_det)
    ...
    import socket
    def _can_connect_to_ip (self, ip, port=3260):
        connected = False
        try:
        ...
        if iscsi_ports:
            for port in iscsi_ports:
                if (port.get('ipv4Enabled') and port.get('iqn') and
                        ...
                        == 'configured' and
                        self._can_connect_to_ip (port['ipv4Data']['ipv4AddressData']['ipv4Address'], port.get('tcpListenPort'))):
                    iscsi_det = {}
                    iscsi_det['ip'] =\
                    ...
                    iscsi_det['priority'] = port['ipv4Data']['ipv4OutboundPacketPriority']['value']
    ...
    def _get_iscsi_portal_for_vol(self, volume, portals, anyController=True):
        ...
        if len(possible_portals) != 0:
            portal = possible_portals[0]
            for pp in possible_portals:
                if pp['priority'] > portal['priority']:
                    portal = pp
            return portal
