...
yes | mdadm --create /dev/md2 --level=1 -c64 --raid-devices=2 /dev/sdb /dev/sdc --spare-devices=1 /dev/sde
mkfs.xfs -f /dev/md2

yes | mdadm --create /dev/md3 --level=5 -c64 --raid-devices=7 /dev/sdf /dev/sdg /dev/sdh /dev/sdi /dev/sdj /dev/sdk /dev/sdl --spare-devices=2 /dev/sdm /dev/sdn
mkfs.xfs -f /dev/md3
...
wget -P /etc http://192.168.10.10/rhat_ic/ciac_files/version23/os_configs_23.tar
tar -xvf /etc/os_configs_23.tar -C /etc
...
wget -P /usr/local/lib/python2.7/transcirrus http://192.168.10.10/builds/rhat_ic/version_234_release
sleep 1
tar -xvf /usr/local/lib/python2.7/transcirrus/version_234_release -C /usr/local/lib/python2.7/transcirrus
...
rm /usr/local/lib/python2.7/transcirrus/version_234_release
...
sed -i 's/BRIDGE=\"br-ex\"/#BRIDGE=\"br-ex\"/' /etc/sysconfig/network-scripts/ifcfg-p1p1
sed -i 's/HWADDR=/#HWADDR=/' /etc/sysconfig/network-scripts/ifcfg-p1p1
sed -i 's/HWADDR=/#HWADDR=/' /etc/sysconfig/network-scripts/ifcfg-p1p2
...
ip link set p1p1 promisc on
ip link set p1p2 promisc on
...
echo 'ip link set p1p1 promisc on' >> /transcirrus/promisc
echo 'ip link set p1p2 promisc on' >> /transcirrus/promisc
echo 'ovs-vsctl add-bond br-ex bond2 p1p1 p1p2' >> /transcirrus/promisc
