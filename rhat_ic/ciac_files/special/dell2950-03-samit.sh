...
echo '/dev/sda1 /data/gluster xfs defaults 1 2' >> /etc/fstab
...
sed -i 's/BRIDGE=\"br-ex\"/#BRIDGE=\"br-ex\"/' /etc/sysconfig/network-scripts/ifcfg-p2p1
sed -i 's/HWADDR=/#HWADDR=/' /etc/sysconfig/network-scripts/ifcfg-p2p1
sed -i 's/HWADDR=/#HWADDR=/' /etc/sysconfig/network-scripts/ifcfg-p2p2
...
ip link set p2p1 promisc on
ip link set p2p2 promisc on
...
echo 'ip link set p2p1 promisc on' >> /transcirrus/promisc
echo 'ip link set p2p2 promisc on' >> /transcirrus/promisc
echo 'ovs-vsctl add-bond br-ex bond2 p2p1 p2p2' >> /transcirrus/promisc
