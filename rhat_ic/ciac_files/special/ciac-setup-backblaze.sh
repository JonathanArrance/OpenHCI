...
wget -P /etc http://192.168.10.10/rhat_ic/ciac_files/nightly/os_configs_nightly.tar
tar -xvf /etc/os_configs_nightly.tar -C /etc
...
sed -i 's/BRIDGE=\"br-ex\"/#BRIDGE=\"br-ex\"/' /etc/sysconfig/network-scripts/ifcfg-eth0
...
echo 'ip link set eth0 promisc on' >> /transcirrus/promisc
echo 'ip link set eth1 promisc on' >> /transcirrus/promisc
echo 'ovs-vsctl add-bond br-ex bond2 eth0 eth1' >> /transcirrus/promisc
