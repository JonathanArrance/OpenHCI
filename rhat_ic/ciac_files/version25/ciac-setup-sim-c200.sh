...
# wget known good packages and rpm install them
...
rpm -Uvh --nodeps --force --nosignature /root/repos.prod.transcirrus.com/transcirrus/CentOS/6/7/TCpackages/openstack-neutron-openvswitch-2014.1.5-1.el6.noarch.rpm
...
#um-config-manager --add-repo /root/glusterfs-epel-35.repo
...
#install monit - installed from local repo
...
#make sure all services belong to their user
#rm -rf /etc/neutron
#rm -rf /etc/nova
#rm -rf /etc/glance
#rm -rf /etc/cinder
...
echo 'export ac_cv_func_malloc_0_nonnull=yes' >> /etc/rc.local
