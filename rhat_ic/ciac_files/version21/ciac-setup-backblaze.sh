...
wget -P /usr/local/lib/python2.7/transcirrus http://192.168.10.10/builds/rhat_ic/version_2_release
sleep 1
tar -xvf /usr/local/lib/python2.7/transcirrus/version_2_release -C /usr/local/lib/python2.7/transcirrus
...
rm /usr/local/lib/python2.7/transcirrus/version_2_release
...
yes | mdadm --create /dev/md2 --level=5 -c64 --raid-devices=3 /dev/sdc /dev/sdd /dev/sde --spare-devices=1 /dev/sdf
mkfs -t xfs /dev/md2

yes | mdadm --create /dev/md3 --level=5 -c64 --raid-devices=5 /dev/sdg /dev/sdh /dev/sdi /dev/sdj /dev/sdk --spare-devices=1 /dev/sdl
mkfs -t xfs /dev/md3
...
echo '/dev/md2 /data/gluster xfs defaults 1 2' >> /etc/fstab
mount -a && mount

echo '/dev/md3 /data/gluster-spindle xfs defaults 1 2' >> /etc/fstab
mount -a && mount
...
#create the cinder spindle volume
...
sed -i 's/TYPE=\"Ethernet\"/TYPE=\"OVSPort\"/' /etc/sysconfig/network-scripts/ifcfg-eth0
sed -i 's/BRIDGE=\"br-ex\"/OVS_BRIDGE=\"br-ex\"/' /etc/sysconfig/network-scripts/ifcfg-eth0
...
echo 'ip link set eth1 promisc on' >> /transcirrus/promisc
echo 'ovs-vsctl add-port br-ex eth1' >> /transcirrus/promisc
