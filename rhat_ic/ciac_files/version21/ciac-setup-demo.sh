...
wget -P /etc http://192.168.10.10/rhat_ic/ciac_files/os_configs.tar
...
wget -P /usr/local/lib/python2.7/transcirrus http://192.168.10.10/builds/rhat_ic/version_21_release
sleep 1
tar -xvf /usr/local/lib/python2.7/transcirrus/version_21_release -C /usr/local/lib/python2.7/transcirrus
...
rm /usr/local/lib/python2.7/transcirrus/version_21_release
...
#parted -s -a optimal /dev/md0 mklabel gpt -- mkpart primary xfs 1 -1
yes | mdadm --create /dev/md0 --level=0 -c64 --raid-devices=2 /dev/sdb /dev/sdc
mkfs -t xfs /dev/md0

parted -s -a optimal /dev/sdd mklabel gpt -- mkpart primary xfs 1 -1
mkfs -t xfs /dev/sdd1
...
#demo hack
#mount -t xfs /dev/md0 /data/gluster

#save old mdadm config
mv /etc/mdadm.conf /etc/mdadm.orig
#save the md conf
mdadm --detail --scan >> /etc/mdadm.conf

echo '/dev/md0 /data/gluster xfs defaults 1 2' >> /etc/fstab

#echo '/dev/sdc1 /data/gluster xfs defaults 1 2' >> /etc/fstab
echo '/dev/sdd1 /data/gluster-spindle xfs defaults 1 2' >> /etc/fstab
mount -a && mount
...
#save the iptable settings
/sbin/service iptables save

#add libffi-devel
wget -P /root http://192.168.10.10/rhat/ciac_files/libffi-devel-3.0.5-3.2.el6.x86_64.rpm
rpm -ivh /root/libffi-devel-3.0.5-3.2.el6.x86_64.rpm

#build swift 1.8 - 
wget -P /root http://192.168.10.10/rhat/ciac_files/swift-1.8.0.tar.gz
tar -zxvf /root/swift-1.8.0.tar.gz -C /root
cd /root/swift-1.8.0
python ./setup.py build
python ./setup.py install


wget -P /root http://192.168.10.10/rhat/gluster/gluster-swift-1.8.0-7.tar
tar -xvf /root/gluster-swift-1.8.0-7.tar -C /root
cd /root/gluster-swift-1.8.0-7
python ./setup.py install
cd /root/gluster-swift-1.8.0-7/etc/
...
mv /root/gluster-swift-1.8.0-7/etc/account-server.conf /etc/swift/account-server.conf
mv /root/gluster-swift-1.8.0-7/etc/container-server.conf /etc/swift/container-server.conf
mv /root/gluster-swift-1.8.0-7/etc/object-server.conf /etc/swift/object-server.conf
mv /root/gluster-swift-1.8.0-7/etc/proxy-server.conf /etc/swift/proxy-server.conf
mv /root/gluster-swift-1.8.0-7/etc/swift.conf /etc/swift/swift.conf
...
#need this for gluster
echo '172.24.24.10:cinder-volume-ssd' >> /etc/cinder/shares.conf
...
#need this for gluster
echo '172.24.24.10:cinder-volume-spindle' >> /etc/cinder/shares_two.conf
chmod 775 /data/gluster-spindle
chown -R cinder:cinder /data/gluster-spindle

#create a volume for instances sotred on disk - used for Nova instance_path variable
