...
HOSTNAME=sn-hc-${NODETWO}

#ciac node id
NODEID=003-${NODEONE}-${NODETWO}
...
wget -P /etc http://192.168.10.10/rhat_ic/sn_files/version21/sn_configs_spindle_21.tar
tar -xvf /etc/sn_configs_spindle_21.tar -C /etc
...
echo '/dev/sda1 /data/gluster-'${HOSTNAME}' xfs defaults 1 2' >> /etc/fstab
mount -a && mount
...
echo "DISK_TYPE='spindle'" >> /usr/local/lib/python2.7/transcirrus/common/config.py
...
wget -P /etc/init.d http://192.168.10.10/rhat_ic/sn_files/sn_zero_connect
