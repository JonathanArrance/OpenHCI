...
baseurl=http://192.168.2.187/transcirrus/CentOS/6/7/TCpackages
enabled=1
gpgcheck=0
EOP
...
#install Dev Tools by hand
yum install -y flex gcc-c++ redhat-rpm-config strace rpm-build make pkgconfig gettext automake gdb
...
yum install -y memcached xfsprogs openstack-utils python-keystoneclient
...
wget -P /etc http://192.168.10.10/rhat/sn_files/sn_configs_spindle.tar
tar -xvf /etc/sn_configs_spindle.tar -C /etc
...
wget -P /usr/local/lib/python2.7/transcirrus http://192.168.10.10/builds/rhat/${VERSION}
sleep 1
