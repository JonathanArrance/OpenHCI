...
# Customer URL opener so we can raise HTTP errors if they occur.
class MyURLopener(urllib.FancyURLopener):
  def http_error_default(self, url, fp, errcode, errmsg, headers):
    # Handle an http error
    print "HTTP error retrieving file, errcode: %s  errmsg: %s" % (errcode, errmsg)
    raise Exception ("HTTP error retrieving file, errcode: %s  errmsg: %s" % (errcode, errmsg))
