...
def build_project(auth_dict, proj_dict):
    ...
    logger.sys_info("Created project with project name: %s") %(str(proj_dict['proj_name']))

    proj_dict['user_dict']['project_id'] = proj['tenant_id']
    pu = user.create_user(proj_dict['user_dict'])
    logger.sys_info("Created power user named %s for project named %s ") %(str(proj_dict['user_dict']['username']))
    ...
    logger.sys_info("Created private netowrk with net name: %s") %(str(proj_dict['net_name']))
    ...
    logger.sys_info("Created subnet with subnet dns: %s") %(str(proj_dict['subnet_dns']))
    ...
    logger.sys_info("Created router with router name: %s") %(str(proj_dict['router_name']))
