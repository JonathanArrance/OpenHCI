from transcirrus.component.keystone.keystone_tenants import tenant_ops
from transcirrus.component.keystone.keystone_users import user_ops
from transcirrus.component.neutron.network import neutron_net_ops
from transcirrus.component.nova.server import server_ops
from transcirrus.component.neutron.layer_three import layer_three_ops


def build_complete_project(auth_dict, proj_name, user_dict, net_name, subnet_dns, sec_group_dict, sec_keys_name, router_name):
    """
    DESC: Build complete project.
    INPUT: auth_dict
           proj_dict - proj_name - req
                     - user_dict - username - req
                                 - password - req
                                 - userrole - must be "pu"
                                 - email - req
                                 - project_id - leave NULL
                     - net_name - req
                     - subnet_dns - req
                     - sec_group_dict - ports[] - op
                                      - group_name - req
                                      - group_desc - req
                                      - project_id - leave NULL
                     - sec_keys_name - req
                     - router_name - req
    OUTPUT:
    ACCESS:
    NOTES:
    """
    tenant = tenant_ops(auth_dict)
    user = user_ops(auth_dict)
    neutron_net = neutron_net_ops(auth_dict)
    nova = server_ops(auth_dict)
    neutron_router = layer_three_ops(auth_dict)

    proj = tenant.create_tenant(proj_name)

    user_dict['project_id'] = proj['tenant_id']
    pu = user.create_user(user_dict)

    net_dict = {'net_name': net_name,'admin_state':"true", 'shared':"true",'project_id':proj['tenant_id']}
    net = neutron_net.add_private_network(net)

    subnet_dict = {'net_id': net['net_id'],'subnet_dhcp_enable':'true','subnet_dns': subnet_dns}
    subnet = neutron_net.add_net_subnet(subnet_dict)

    sec_group_dict['project_id'] = proj['tenant_id']
    sec_group = nova.create_sec_group(sec_group_dict)

    sec_keys_dict = {'key_name': sec_keys_name, 'project_id': proj['tenant_id']}
    sec_keys = nova.create_sec_keys(sec_keys_dict)

    router_dict = {'router_name': router_name, 'project_id': proj['tenant_id']}
    router = neutron_router.add_router(router_dict)

    inside_port_dict = {'router_id': router['router_id'], 'subnet_name': subnet['subnet_name'], 'project_id': proj['tenant_id']}
    inside_port = neutron_router.add_router_internal_interface(inside_port_dict)

    outside_port_dict = {'router_id': router['router_id'], 'ext_net_id': ????}
    outside_port = neutron_router.add_router_gateway_interfac(outside_port_dict)