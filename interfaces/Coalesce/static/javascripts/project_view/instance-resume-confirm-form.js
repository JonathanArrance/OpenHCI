$(function() {  
	...
	$(function() {
		...
		$( "#instance-resume-confirm-form" ).dialog({
			...
			buttons: {
				"Confirm": function() {
					...
					message.showMessage('notice', 'Resuming Instance');		// Flag notice

					// Create loader
					var confirmedActionSelector = '#'+confirmedId+'-actions-cell > .resume-instance';	// Target clicked action link
					var confirmedActionHtml = '<a href="#" class="resume-instance">resume</a>';			// Copy the html that link
					...
					$.getJSON('/server/' + PROJECT_ID + '/' + confirmedId + '/resume_server/')
						.success(function(data){
							...
							// Check if instance was successfully resumed
		...
		// Open modal form when resume-instance button clicked
		$(document).on('click', '.resume-instance', function(){

			instanceId = $(this).parent().parent().attr('id');		// Get the ID of the instance that the user wishes to resume

			// Clear and add instance name to .instance-name span in confirm statement
			var nameSelector = '#'+instanceId+'-name-text';
			$('#instance-resume-confirm-form > p > span.instance-name')
				.empty()
				.append($(nameSelector).text());

			$( "#instance-resume-confirm-form" ).dialog( "open" ); 	// Open resume Confirm Form					
		});
	});
});
