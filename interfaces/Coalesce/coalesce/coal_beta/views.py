...
# ASM changes - TODO change the location of constants and properties accordingly
...
def GetClientIP(request):
    """Gets the clients IP address from request object.
    ...
    """
    for ip_header in constants.IP_HEADERS_ORDERED:
        if request.META.get(ip_header):
            return request.META.get(ip_header)


def GetCookieName(username, hostname):
    """Prepares and returns the name of cookie to be set in the response.

    Args:
    username: String representing the name of the current user.
    hostname: String representing the name of the host.

    Returns:
    String (hashed by md5/sha1) representing the cookie name.
    """
    cookie_key = properties.COOKIE_NAME % (username, hostname)
    return hashlib.md5(cookie_key).hexdigest()


def MakeRequestToApersona(url, data):
    """Makes a server side call to the aPersona authentication system.

    Args:
    url: String representing the URL to make the call.
    data: Dict representing the params that needs to be passed.

    Returns:
    HttpResponse from aPersona authentication system.
    """
...
def dashboard(request):
    ...
    aPersona = extras.is_apersona_up()
    try:
                ...
                if auth != None:
                    if sa['user_level'] > 0:
                        ...
                        return render_to_response('coal/welcome.html',
                                                  RequestContext(request, {"project_admin": project_admin, 'aPersona': aPersona}))
                else:
                    return render_to_response('coal/welcome.html',
                                              RequestContext(request, {"providers": tpa_providers, 'aPersona': aPersona}))
                    ...
                    #     return render_to_response('coal/welcome.html', RequestContext(request, { "providers": tpa_providers}))
            return render_to_response('coal/welcome.html',
                                      RequestContext(request, {"providers": tpa_providers, 'error': "Error: bug", 'aPersona': aPersona}))
        ...
        # if tpa_providers['has_other'] != False:
        #     return render_to_response('coal/welcome.html', RequestContext(request, { "providers": tpa_providers}))
        return render_to_response('coal/welcome.html', RequestContext(request, {"providers": tpa_providers, 'error': "Error: %s" % e, 'aPersona': aPersona}))
...
def create_security_group(request, groupname, groupdesc, ports, transport, project_id):
    try:
        ...
        auth = request.session['auth']
        so = server_ops(auth)
        create_sec = {'group_name': groupname, 'group_desc':groupdesc, 'ports': port_array, 'transport': transport, 'project_id': project_id}
...
def update_security_group(request, groupid, project_id, ports, enable_ping, transport):
    ...
    try:
        ...
        auth = request.session['auth']
        so = server_ops(auth)
        update_sec = {'group_id': groupid, 'enable_ping': enable_ping, 'ports': port_array, 'project_id': project_id, 'transport': transport, 'update':'true'}
...
def get_asm_lic_key():
      #TODO : obtain appropriate ASM Lic Key based on user type, hardcoding to USER type for now
      return properties.LICENSE_KEY_USER;
...
def login(request):
    ...
    if request.method == 'GET':
        ...
        return render_to_response('coal/welcome.html',RequestContext(request, {"error": error}))
